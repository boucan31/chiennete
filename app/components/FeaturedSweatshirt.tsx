...
interface FeaturedSweatshirtProps {
...
const availableColors = ['blanc', 'gris', 'noir'];
...
export default function FeaturedSweatshirt({ initialProduct }: FeaturedSweatshirtProps) {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      try {
        ...
        // Encoder le handle pour l'URL (les espaces et caractères spéciaux doivent être encodés)
        const encodedHandle = encodeURIComponent(handle);
        console.log(`Fetching product with handle: "${handle}" (encoded: "${encodedHandle}")`);
